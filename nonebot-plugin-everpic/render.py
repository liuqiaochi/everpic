...
def render_help_image() -> bytes:
    ...
    lines = [
        ...
        ("body", "  举例: everpic删词 3"),
        ("blank", ""),
        ("heading", "【HQ 高清升级】（消耗10积分，超级管理员免费）"),
        ("body", "  回复本机器人生成的图片 + 文本 HQ"),
        ("body", "  对已有图片做高清升级（upscale）"),
        ("body", "  举例: 回复画图的图片 + HQ"),
        ("body", "  注意: 原图生成后 7 天内可 HQ"),
        ("blank", ""),
        ("heading", "【队列查询】"),
        ...
        ("body", "  everpic加禁词 词 → 添加禁词（超级管理员）"),
        ("body", "  everpic删禁词 词 → 删除禁词（超级管理员）"),
        ("body", "  everpic查禁词 [关键词] → 查看/搜索禁词（超级管理员）"),
